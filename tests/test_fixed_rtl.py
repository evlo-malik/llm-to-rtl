...
@pytest.mark.parametrize("shape,scaled", [((1,1),False), ((7,5),False), ((35,9),True)])
def test_fixed_rtl(tmp_path, monkeypatch, sim, shape, scaled):
    ...
    runner.build(sources=[rtl],hdl_toplevel='fixed',build_dir=build,
                 timescale=('1ns','1ps'),build_args=['-Wno-fatal'] if sim=='verilator' else [])
    runner.test(hdl_toplevel='fixed',test_module='test_fixed',test_dir=ROOT/'tb',build_dir=build,
