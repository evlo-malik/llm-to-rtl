...
@pytest.mark.skipif(not (MODEL/'model.safetensors').exists(),reason='download example checkpoint first')
def test_pretrained_gpt2_against_transformers():
    ...
    state={k:torch.from_numpy(v) for k,v in st.items() if not k.endswith(".attn.masked_bias")}
    ...
    print('float max error:',np.abs(actual-expected).max(),'INT8 argmax agreement:',agreement)
