// One token of causal multi-head attention, integer only, with the KV cache inside.
//
// Per (layer, pos): 3D int8 values stream in (q, then k, then v). k and v go into the
// cache at [layer][pos]; then for each head h, over positions i = 0..pos:
//     s[i]  = q_h . k_h[i]                                   int32, serial MAC
//     u[i]  = clip255( ((max s - s[i]) * M0u + 2^(nu-1)) >> nu )   16 per nat, 255 = far
//     e[i]  = LUT[u[i]]  = round(2^16 * exp(-u/16))          17 bits
//     R     = 2^36 / sum e                                    one division per head
//     p[i]  = (e[i] * R) >> 21                                uint16, 2^-15 units
//     o_h[c] = clip8( (sum_i p[i] v_h[i][c]) * M0o + 2^(no-1) >> no )
// and the D values of o stream out, head 0 first. This is quant.softmax_int and
// IntGPT.step in compiler/golden_gpt.py, cycle by cycle. (M0u, nu, M0o, no) come from
// PARAM_FILE, one word per layer: {no[5:0], M0o[15:0], nu[5:0], M0u[15:0]}.
//
// Everything is one multiply per cycle. About 34*(pos+1) + 60 cycles per head.
module attention #(
    ...
    // ---- parameters and LUT ----
    (* ram_style = "logic" *) logic [43:0] params [L];
    logic [16:0] lut [256];
    initial begin
        $readmemh(PARAM_FILE, params);
        $readmemh(LUT_FILE, lut);
    end
    logic [43:0] prm;
    logic [15:0] m0u, m0o;
    logic [5:0]  nu, no;
    assign prm = params[layer];
    assign m0u = prm[15:0];
    assign nu  = prm[21:16];
    ...
    udiv #(.NW(37), .DW(22)) u_div (.clk(clk), .rst_n(rst_n), .start(dv_start),
