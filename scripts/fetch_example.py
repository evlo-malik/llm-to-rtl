...
def main():
    ...
    state = torch.load(
        args.out / "pytorch_model.bin", map_location="cpu", weights_only=True
    )
    # GPT-2 ties the output projection to the token embedding. Clone aliases for safetensors.
    state = {
        k: v.contiguous().clone() for k, v in state.items() if v.is_floating_point()
    }
    ...
    texts = [
        "Hello world. This is a small hardware test.",
        "The quick brown fox jumps over the lazy dog.",
        "Memory and computation are connected by wires.",
        "A model predicts the next token in a sentence.",
        "One two three four five six seven eight.",
        "London is a city. Paris is another city.",
    ]
    (args.out / "calibration.json").write_text(
        json.dumps([tokenizer.encode(t) for t in texts], indent=2) + "\n"
    )
    evaluation = [
        "A red balloon floated above the quiet street.",
        "She opened the book and started reading.",
        "What happens when the weather changes?",
        "Seven birds landed on the wooden fence.",
    ]
    (args.out / "evaluation.json").write_text(
        json.dumps([tokenizer.encode(t) for t in evaluation], indent=2) + "\n"
    )
