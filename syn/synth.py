...
def main():
    ...
    if manifest["scope"] != "full_model" and len(names) != 1:
        parser.error("synthesise a full model or a single selected matrix")
    top = manifest.get("top", names[0])
    ...
    files = " ".join("rtl/" + p.name for p in sorted((out / "rtl").glob("*.sv")))
